...
def preserve_legend(ax: Axes,  # type: ignore
                    patches: List[LegendArtist], # Renamed for clarity: these are the new *artists* to add
...
def plot_as_image(data: VEC_TYPE,
    ...
    for i, data in enumerate(input_data):
        ...
        if 'complex' in str(data.dtype):
            cols = 2
            _col_titles.append(f"{title_num_str}|{v_name}|") if v_name is not None else _col_titles.append(None)
            _col_use_mathjax.append(used_mj)
            _col_images.append(np.abs(data))
            _col_cmaps.append(cmap)

            _col_titles.append(f"{title_num_str}angle({v_name})") if v_name is not None else _col_titles.append(None)
            _col_use_mathjax.append(used_mj)
            angle = np.angle(data)
            _col_images.append(angle)
            _col_cmaps.append(phase_cmap)
        else:
            _col_titles.append(title_num_str + v_name) if v_name is not None else _col_titles.append(None)
            _col_use_mathjax.append(used_mj)
