...
class RichDataFrame:
    ...
    def _add_caption(self):
        ...
        self.table.caption = (f"{'Only the' if len(self.df) > self.row_limit or len(self.df.columns) > self.col_limit else ''}" +
                              ...
                              f" and {'the' if col_text != 'all' else ''} [bold green not dim]{col_text} {self.col_limit if len(self.df.columns) > self.col_limit else str(len(self.df.columns))} columns[/bold green not dim]" +
