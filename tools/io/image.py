...
def alpha_background_grid(
    ...
    primary_color = (parse_color_rgba(primary_color) * 255).astype(np.uint8).tolist()
    secondary_color = (parse_color_rgba(secondary_color) * 255).astype(np.uint8).tolist()
    ...
    second_color_mask = ((np.floor(coords / square_size) % 2) == 1).sum(axis=-1) == 1
